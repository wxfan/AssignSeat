...
using AssignSeat.Models;
 ...
 {
     public class DepartmentOrderStrategy : ISeatAssignmentStrategy
     {
         public void AssignSeats(MeetingRoom room, List<Employee> employees)
         {
             ...
             // Sort employees by department order number
             var sortedEmployees = employees.OrderBy(e => e.Department).ThenBy(e => e.Name).ToList();
             ...
             // Assign seats
             Console.WriteLine($"Assigning seats in {room.RoomName} by department order:");
